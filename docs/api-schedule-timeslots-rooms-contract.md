...
Справочник временных слотов: день недели (1–7), время начала и окончания. Используется при создании уроков (в контракте уроков не описывается).

### 4.1 Модель данных таймслота (TimeslotDto)

Во всех успешных ответах, где возвращается таймслот или список таймслотов, используется объект **TimeslotDto**:
...
### 3.2 GET /api/schedule/timeslots

**Назначение:** получение списка всех таймслотов (для выбора слота при создании урока, отображения сетки расписания и т.п.).
...
Тело ответа — массив объектов **TimeslotDto**. Список упорядочен по `dayOfWeek`, затем по `startTime`.

**Ошибки:** при невалидном или отсутствующем JWT — `401 Unauthorized` (код `UNAUTHORIZED`); при применении политики на чтение — `403 Forbidden` (код `FORBIDDEN`).
...
### 3.4 POST /api/schedule/timeslots

**Назначение:** создание нового таймслота.
...
### 4.5 DELETE /api/schedule/timeslots/{id}
