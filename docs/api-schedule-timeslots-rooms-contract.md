...
Справочник **шаблонов времени** (день недели + время начала и окончания). Используется в UI как подсказка при выборе времени урока или слота офферинга: пользователь выбирает слот, время подставляется в сущность (урок/слот офферинга владеет своим временем). При удалении таймслота (одного или всех) **уроки не удаляются**: у уроков, которые ссылались на слот, поле `timeslotId` обнуляется (`null`).
...
**Назначение:** удаление одного таймслота по идентификатору. Уроки, которые ссылались на этот слот, не удаляются — у них поле `timeslotId` устанавливается в `null`.
...
**Примечание:** обновление таймслота (PUT) в API не предусмотрено; при необходимости изменить слот — удалить и создать новый.

---

### 4.7 DELETE /api/schedule/timeslots

**Назначение:** удаление всех таймслотов одним запросом. Уроки, которые ссылались на любой слот, не удаляются — у них поле `timeslotId` устанавливается в `null`. Используется для полной очистки справочника шаблонов времени (например, перед загрузкой нового набора слотов).

**Метод и путь:** `DELETE /api/schedule/timeslots`

**Права доступа:** только **MODERATOR**, **ADMIN**, **SUPER_ADMIN**.

**Параметры:** path и query отсутствуют; тело запроса не передаётся.
...
- **401 Unauthorized** — отсутствует или невалиден JWT; код `UNAUTHORIZED`.
- **403 Forbidden** — нет роли MODERATOR/ADMIN/SUPER_ADMIN; код `FORBIDDEN`.

Эндпоинт не возвращает 404: при отсутствии таймслотов операция считается успешной (ничего не удалять, уроки не менять).
...
### 6.8 Удаление всех таймслотов

**Запрос:** `DELETE /api/schedule/timeslots`

(Заголовок: `Authorization: Bearer <JWT>`; тело запроса отсутствует.)

**Ответ:** `204 No Content`

Тело ответа отсутствует. Все таймслоты удалены; у уроков, которые ссылались на слот, поле `timeslotId` установлено в `null`.
