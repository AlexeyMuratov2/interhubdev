...
2. **Получить список предметов преподавателя**  
   ...
   - Параметр `semesterNo` опционален. Если передан — возвращаются только предметы учебного плана с этим номером семестра (1, 2, 3, …). Если не передан — возвращаются все предметы, где преподаватель назначен (основной преподаватель, слот или offering_teacher).  
...
Эндпоинты возвращают данные только по тем предметам, где **текущий пользователь** назначен преподавателем (основной преподаватель в offering, преподаватель в слоте или в offering_teacher).
...
- Собираются все offerings, где преподаватель назначен (основной преподаватель, слот или offering_teacher).  
