...
5. **Сгенерировать уроки**:
   ...
   Количество созданных уроков = (число слотов офферинга) × **durationWeeks** предмета из учебного плана (с учётом границ семестра). Если уроки по этому офферингу уже есть, используйте **POST /api/offerings/{offeringId}/regenerate-lessons?semesterId={semesterId}** (старые удалятся, создадутся заново).

### 2.3 Итог

- **До генерации** нужны: группа с планом, предметы плана (с durationWeeks), семестр, офферинги, у каждого офферинга — хотя бы один слот (день недели + время + тип занятия). Учитель и комната могут быть на уровне офферинга или на уровне слота.
...
- **Поведение системы:** при удалении офферинга **в одном запросе** автоматически:
  ...
  3. Удаляются **все привязки учителей** к офферингу (OfferingTeacher).
  4. Удаляется сам офферинг.
...
|------------------|----------------------------------|
| Офферинг | Все слоты офферинга, все привязки учителей офферинга, все уроки по этому офферингу |
...
### 4.3 OfferingTeacherDto

Привязка учителя к офферингу с ролью (лекции, практика, лабы).

| Поле | Тип | Описание |
|------|-----|----------|
| `id` | UUID | Идентификатор привязки |
| `offeringId` | UUID | Id офферинга |
...
**Назначение:** удалить офферинг. При удалении автоматически удаляются все слоты этого офферинга, привязки учителей к офферингу и **все уроки**, созданные по этому офферингу (в модуле расписания). Подробнее см. раздел 3.3.
...
К одному офферингу можно привязать нескольких учителей с разными ролями (лекции, практика, лабы).

### 6.1 GET /api/offerings/{offeringId}/teachers

**Назначение:** получить список учителей, привязанных к офферингу (для отображения на экране офферинга).
...
**Успешный ответ:** `200 OK` — массив объектов **OfferingTeacherDto**.

**Ошибки:** при несуществующем offeringId ответ может быть пустой массив или 404 в зависимости от реализации.

---

### 6.2 POST /api/offerings/{offeringId}/teachers

**Назначение:** добавить учителя к офферингу с указанием роли (LECTURE, PRACTICE, LAB).

**Метод и путь:** `POST /api/offerings/{offeringId}/teachers`
...
| Поле | Тип | Обязательность | Описание |
|------|-----|----------------|----------|
| `teacherId` | UUID | обязательно | Id профиля учителя (из GET /api/account/teachers — `profile.id`) |
| `role` | string | обязательно | Роль: `LECTURE`, `PRACTICE`, `LAB` |

**Успешный ответ:** `201 Created` — объект **OfferingTeacherDto**.

**Ошибки:** `404` — офферинг или учитель не найдены; `409` — такая пара учитель+роль уже есть у офферинга.

---

### 6.3 DELETE /api/offerings/teachers/{id}

**Назначение:** удалить привязку учителя к офферингу (id — это id записи OfferingTeacher, а не teacherId).

**Метод и путь:** `DELETE /api/offerings/teachers/{id}`

**Роли:** MODERATOR, ADMIN, SUPER_ADMIN.

**Параметр пути:** `id` — UUID записи привязки (OfferingTeacherDto.id).

**Успешный ответ:** `204 No Content`.

**Ошибки:** `404 Not Found` — привязка не найдена.
...
**Назначение:** создать уроки по одному офферингу на выбранный семестр. Количество уроков = (число слотов офферинга) × durationWeeks предмета учебного плана (с обрезкой по датам семестра). У каждого урока — конкретная дата и время (по слотам и неделям семестра); у урока заполняется **offeringSlotId** (для типа занятия и преподавателя на UI). Вызывать только после того, как у офферинга есть хотя бы один слот.
...
| `semesterId` | UUID | да (query) | Id семестра (из GET /api/academic/semesters/... или GET /api/academic/semesters/current) |
...
**Назначение:** создать уроки по **всем** офферингам группы на выбранный семестр. Для каждого офферинга группы, у которого есть хотя бы один слот и найден предмет учебного плана, генерируются уроки. Офферинги без слотов или с отсутствующим предметом плана пропускаются. Удобно для массовой генерации «вся группа на семестр».
...
| `semesterId` | UUID | да (query) | Id семестра |
...
**Назначение:** удалить все существующие уроки по этому офферингу и заново сгенерировать их на выбранный семестр (по текущим слотам и durationWeeks). Используется, когда изменились слоты или нужно «пересчитать» уроки.

**Метод и путь:** `POST /api/offerings/{offeringId}/regenerate-lessons?semesterId={semesterId}`

**Роли:** MODERATOR, ADMIN, SUPER_ADMIN.

**Параметры:** те же, что у generate-lessons (offeringId в path, semesterId в query).
...
(24 урока, например: 2 слота × 12 недель по учебному плану.)
