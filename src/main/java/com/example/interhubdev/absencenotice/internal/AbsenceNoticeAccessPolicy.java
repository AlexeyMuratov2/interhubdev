...
/**
 * Access policy for absence notice operations (e.g. teacher respond).
 */
final class AbsenceNoticeAccessPolicy {

    private final UserApi userApi;
    private final TeacherApi teacherApi;
    private final OfferingApi offeringApi;

    AbsenceNoticeAccessPolicy(UserApi userApi, TeacherApi teacherApi, OfferingApi offeringApi) {
    ...
    void ensureCanManageSession(UUID userId, LessonDto lessonDto) {
        GroupSubjectOfferingDto offering = offeringApi.findOfferingById(lessonDto.offeringId())
                .orElseThrow(() -> AbsenceNoticeErrors.offeringNotFound(lessonDto.offeringId()));
        ensureCanManageOffering(userId, offering);
    }

    private void ensureCanManageOffering(UUID userId, GroupSubjectOfferingDto offering) {
        UserDto user = userApi.findById(userId)
                .orElseThrow(() -> AbsenceNoticeErrors.forbidden("User not found"));
        ...
        if (user.hasRole(Role.TEACHER)) {
            TeacherDto teacher = teacherApi.findByUserId(userId)
                    .orElseThrow(() -> AbsenceNoticeErrors.forbidden("User does not have a teacher profile"));
            ...
            if (!isAssignedTeacher) {
                throw AbsenceNoticeErrors.forbidden("Only teachers assigned to this offering can respond to notices");
            }
            return;
        }

        throw AbsenceNoticeErrors.forbidden("Only teachers or administrators can respond to notices");
    }
}
